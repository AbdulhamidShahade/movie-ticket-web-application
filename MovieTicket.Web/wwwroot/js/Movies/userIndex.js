...
function showMovieData(movieName, movieDescription, startDate, endDate, movieId, moviePicture, price, cinemaName, publishYear, length, categoryNames, rating) {
    ...
    document.getElementById('movie-details').setAttribute('href', `http://movieticketwebapplication-dev.us-east-1.elasticbeanstalk.com/Movies/Details/${movieId}`);
    document.getElementById('movie-picture').setAttribute('src', moviePicture);
    document.getElementById('shopping-cart').setAttribute('href', `http://movieticketwebapplication-dev.us-east-1.elasticbeanstalk.com/Orders/AddItemToShoppingCart/${movieId}`);
